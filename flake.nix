{
  description = "rqmd - on-device hybrid document search (single static binary)";
  ...
  outputs = { self, nixpkgs, flake-utils }:
    flake-utils.lib.eachDefaultSystem (system:
      let
        ...
        # Native build inputs needed to compile rqmd (llama-cpp-2 uses CMake + C/C++).
      ...
      in
      {
        # Development shell: full Rust toolchain + native deps for building rqmd.
        ...
        devShells.default = pkgs.mkShell {
          ...
          shellHook = ''
            echo "rqmd development shell"
            echo "  cargo build --workspace               — debug build"
            echo "  cargo build --profile dist -p rqmd-cli  — release binary → target/dist/rqmd"
            echo "  cargo run --bin rqmd -- <command>     — run from source"
          '';
        };
      }
    );
