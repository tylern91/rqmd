...
pub fn run_doctor(index_dir: &Path) -> Result<()> {
    ...
    if db_path.exists() {
        ...
        // `count_docs_needing_embed`). Mixed fingerprints degrade vector search
        // silently — warn and point at `--rebuild`.
        let breakdown = db::fingerprint_breakdown(&s.db).unwrap_or_default();
        if breakdown.len() > 1 {
            let default_cfg = rqmd_llm::LlamaCppConfig::default();
            let expected = core_store::expected_embed_fingerprint(
                &default_cfg.embed_repo,
                &default_cfg.embed_file,
            );
            println!("\n  \x1b[33m⚠ Mixed embedding fingerprints detected\x1b[0m — vectors were generated by more than one model/chunking config:");
